...
function Get-RelaunchArgs {
  ...
  foreach ($u in $args) { $out.Add([string]$u) }
  return $out.ToArray()
}
...
# --------------------- LOG SEMPRE (MESMO DIRETORIO DO SCRIPT) ---------------------
...
$logName = "pendrive-bootavel_{0}.txt" -f (Get-Date -Format 'dd-MM-yyyy_HH-mm-ss')
...
function Wait-AnyKey([string]$Msg = "Pressione qualquer tecla para finalizar...") {
  Write-Host ""
  Write-Host $Msg -ForegroundColor Yellow
  try { [void][Console]::ReadKey($true) } catch { Read-Host "Pressione ENTER para finalizar" | Out-Null }
}
...
function Get-VolumeLabel([string]$fs) {
  if ($fs -eq 'FAT32') { return 'WINBOOTFAT' }
  return 'WINBOOTNTFS'
}
...
function Select-IsoPathFileDialog {
  ...
  try { Add-Type -AssemblyName System.Windows.Forms | Out-Null } catch { return $null }
...
function Select-UsbDiskInteractive {
  ...
  # REQ: se nao houver USB, aguarda tecla e finaliza (sem "matar seco")
  if (@($disks).Count -eq 0) {
    Write-Warn "Nenhum dispositivo removivel (USB/UAS) encontrado."
    Wait-AnyKey
    return $null
  }
...
function Get-InstallImageInfo([string]$isoRoot) {
  ...
  $swmGlob = Join-Path $isoRoot "sources\install*.swm"

  $install = $null
  $swm = @()
  try { $swm = @(Get-ChildItem -Path $swmGlob -ErrorAction SilentlyContinue) } catch { $swm = @() }

  if ($swm.Count -gt 0) {
    Write-Host ("Arquivos encontrados: install*.swm ({0} arquivos)" -f $swm.Count)
    return @{
      File      = $swm[0]
      IsWim     = $false
      IsEsd     = $false
  ...
  else { throw "Nao foi possivel encontrar install.wim, install.esd ou install*.swm em sources da ISO." }
...
function Invoke-External([string]$FilePath, [string[]]$ArgumentList, [string]$FailMessage) {
  & $FilePath @ArgumentList
  $rc = $LASTEXITCODE
  if ($rc -ne 0) { throw ("{0} ExitCode={1}" -f $FailMessage, $rc) }
}
...
function Convert-EsdToWimAllIndexes([string]$EsdPath, [string]$WimOutPath, [string]$ScratchDir) {
  ...
  $out = & dism.exe /English /Get-WimInfo "/WimFile:$EsdPath" 2>&1
  $text = ($out | Out-String)
  ...
  foreach ($idx in $indexes) {
    ...
    $dismArgs = @(
      '/English','/Export-Image',
      "/SourceImageFile:$EsdPath",
      "/SourceIndex:$idx",
      "/DestinationImageFile:$WimOutPath",
      '/Compress:Max','/CheckIntegrity',
      "/ScratchDir:$ScratchDir"
    )
    Invoke-External -FilePath "dism.exe" -ArgumentList $dismArgs -FailMessage ("Falha ao exportar index {0} do ESD para WIM." -f $idx)
  }
...
function Copy-FilesToUsb([string]$IsoRoot, [string]$UsbRoot, [string[]]$ExcludeFiles) {
  ...
  $roboArgs = @($src, $dst) + $opts

  if ($ExcludeFiles -and $ExcludeFiles.Count -gt 0) {
    $roboArgs += "/XF"
    foreach ($xf in $ExcludeFiles) { $roboArgs += $xf }
  }
...
function Split-WimToSwm([string]$SourceWim, [string]$DestSwm, [int]$FileSizeMB) {
  ...
  $dismArgs = @('/English','/Split-Image',"/ImageFile:$SourceWim", "/SWMFile:$DestSwm", "/FileSize:$FileSizeMB")
  Invoke-External -FilePath "dism.exe" -ArgumentList $dismArgs -FailMessage "DISM Split-Image falhou."
}
...
function Apply-Bootsect([string]$IsoRoot, [string]$UsbRoot) {
  ...
  if (Test-Path $bootsectIso) {
    Invoke-External -FilePath $bootsectIso -ArgumentList @('/nt60', $UsbRoot) -FailMessage "bootsect falhou (ISO)."
  ...
  if (Test-Path $bootsectSys) {
    Invoke-External -FilePath $bootsectSys -ArgumentList @('/nt60', $UsbRoot) -FailMessage "bootsect falhou (System32)."
...
try {
  ...
  # LOG SEMPRE (no diretorio do script)
  ...
  if ($PSBoundParameters.ContainsKey('DiskNumber')) {
    ...
    if (-not $picked) { return } # ja aguardou tecla e finalizou de forma "suave"
  ...
  if ($img.IsSwm) {
    Write-Ok "ISO ja contem install*.swm. Nao e necessario dividir/converter."
  }
  elseif ($FileSystem -eq 'FAT32') {
    if ($img.IsWim -and $img.SizeBytes -ge 4GB) {
      ...
      Write-Ok "Arquivo install.* cabe em FAT32. Divisao nao necessaria, copia direta."
    }
  } else {
    Write-Ok "Destino NTFS. Divisao nao necessaria, copia direta."
  }
